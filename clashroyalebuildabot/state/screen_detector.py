...
class ScreenDetector:
    ...
    def _calculate_screen_hashes(self):
        ...
        for i, name in enumerate(SCREEN_CONFIG.keys()):
            path = os.path.join(DATA_DIR, "images/screen", f"{name}.png")
    ...
    def run(self, image):
        crop_hashes = np.array([
            np.array(image.crop(v["bbox"]).resize((self.hash_size, self.hash_size), Image.BILINEAR)).flatten()
            for v in SCREEN_CONFIG.values()
        ])
        hash_diffs = np.mean(np.abs(crop_hashes - self.screen_hashes), axis=1)
        return "in_game" if min(hash_diffs) > self.threshold else list(SCREEN_CONFIG.keys())[np.argmin(hash_diffs)]
