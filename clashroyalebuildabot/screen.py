...
def check_emulator_properties():
    ...
    if resolution_correct and density_correct:
        ...
        logger.info(
            "ClashRoyaleBuildABot can now be started with `python main.py`."
        )
    else:
        logger.info("The emulator does not have the correct properties.")
        if not resolution_correct:
            logger.info(f"Current resolution: {resolution}")
        if not density_correct:
            logger.info(f"Current density: {density}")
