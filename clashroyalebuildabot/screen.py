    ...
    def __init__(self):
        try:
            ...
            if "no devices/emulators found" in error_output:
                ...
                logger.critical("Multiple devices/emulators found. Please specify a target device using the -s option.")
            else:
    ...
    def take_screenshot(self):
        ...
        try:
            ...
            try:
                ...
                # Attempt to create image with header removed
                try:
                    img = Image.frombuffer('RGB', (self.width, self.height), screenshot_bytes)
                    return img.resize((SCREENSHOT_WIDTH, SCREENSHOT_HEIGHT), Image.BILINEAR)
                except Exception as e:
